...
function initMap() {
    ...
    geocoder.geocode({ 'address': address2 }, function(results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
            ...
            // Place marker at the coordinates
            var carMarker2 = new google.maps.Marker({
                position: location,
                map: map,
                title: 'Car 2'
            });
            ...
            // Add info window that pops up when you scroll over
            var infowindow = new google.maps.InfoWindow({
                content: '<div style="text-align: center;">Car 2</div><br>Address: 944 East Gorham St'
            });

            // Show info window when mouseover marker
            carMarker2.addListener('mouseover', function() {
                infowindow.open(map, carMarker2);
            });

            // Hide info window when mouseout marker
            carMarker2.addListener('mouseout', function() {
                infowindow.close();
            });
    ...
    // Set marker for start point to null to suppress it
    directionsRenderer.setOptions({
        markerOptions: {
            visible: false // Hide the start marker
        }
    });
    ...
    document.getElementById("start").addEventListener("change", function () {
        // Check if the "end" HTML input has a value
